...
campsiteRouter
  ...
  .put(authenticate.verifyUser, (req, res, next) => {
    //Return the campsite by using the ID in the URL
    Campsite.findById(req.params.campsiteId)
      .then((campsite) => {
        //testing for the values coming in on the requests
        console.log(req.user._id);
        console.log(campsite.comments.id(req.params.commentId).author);
        //Checking if you have a campsite with this Id
        if (campsite && campsite.comments.id(req.params.commentId)) {
          //checking to make sure that the author of the comment is the one who wants to change it
          if (
            campsite.comments
